...
export async function setupDatabase() {
  ...
  const { error: gamesError } = await supabase.rpc("create_games_table_if_not_exists", {
    sql: `
      ...
      -- Add is_team_game column if it doesn't exist
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'games' AND column_name = 'is_team_game'
        ) THEN
          ALTER TABLE games ADD COLUMN is_team_game BOOLEAN DEFAULT false;
        END IF;
      END $$;
    `,
  })
